...
namespace AzuAutoStore
{
    ...
    public static class RegisterAndCheckVersion
    {
        private static void Prefix(ZNetPeer peer, ref ZNet __instance)
        {
            ...
            AzuAutoStorePlugin.AzuAutoStoreLogger.LogDebug("Registering version RPC handler");
            peer.m_rpc.Register($"{AzuAutoStorePlugin.ModName}_VersionCheck", new Action<ZRpc, ZPackage>(RpcHandlers.RPC_AzuAutoStore_Version));
    ...
    public static class RemoveDisconnectedPeerFromVerified
    {
        private static void Prefix(ZNetPeer peer, ref ZNet __instance)
        {
            ...
            AzuAutoStorePlugin.AzuAutoStoreLogger.LogInfo($"Peer ({peer.m_rpc.m_socket.GetHostName()}) disconnected, removing from validated list");
        ...
        public static void RPC_AzuAutoStore_Version(ZRpc rpc, ZPackage pkg)
        {
            ...
            else
            {
                if (!ZNet.instance.IsServer())
                {
                    ...
                    AzuAutoStorePlugin.AzuAutoStoreLogger.LogInfo("Received same version from server!");
                }
                else
                {
                    // Add client to validated list
                    AzuAutoStorePlugin.AzuAutoStoreLogger.LogInfo($"Adding peer ({rpc.m_socket.GetHostName()}) to validated list");
